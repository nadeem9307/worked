...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<header class="entry-header">
		<?php
		if ( is_singular() ) :
			the_title( '<h1 class="entry-title xf">', '</h1>' );
		else :
		...
		<?php
			the_content( sprintf(
				wp_kses(
					...
					__( 'Continue reading<span class="screen-reader-text"> "%s"</span>', 'cash-for-apartment' ),
					array(
						'span' => array(
							'class' => array(),
						),
			...
			wp_link_pages( array(
				'before' => '<div class="page-links">' . esc_html__( 'Pages:', 'cash-for-apartment' ),
				'after'  => '</div>',
			) );
		?>
	</div><!-- .entry-content -->
